        ...
        for i in range(self.num_of_step + 1):
            for j in range(env.uav_num):
                if CONTINUE_CHARGE[j][i] > 0:
                    f.writelines('uav-' + str(j) + ' continuous_charge=' + str(i) + ' frequncy=' + str(
                        CONTINUE_CHARGE[j][i]) + '\n')
        f.writelines('\n--------------*------------------\n')
        for i in range(env.uav_num):
            for info in env.uav_charge_trace[i]:
                f.writelines(
                    'uav-' + str(i) + ' step ' + str(info[0]) + ' pos ' + str(
                        info[1]) + ' charge ' + str(info[2]) + ' cur_energy ' + str(
                        info[3]) + '\n')
        f.close()
